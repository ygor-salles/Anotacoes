...
- Verificar se nenhum processo ou outro banco esteja executando na mesma porta definida na variável de ambiente `BD_PORT`, caso não tenha basta agora executar o comando `docker-compose up -d` que uma nova imagem do docker com o banco postgresql estará executando para ser utilizada posteriormente na sua aplicação. Você poderá visualizar o schema do banco postgresql com algum database-studio como `dbeaver` ou `beekeeper` setando as mesmas credenciais definidas em `.env`

## Instalando ORM e dependencias para se conectar ao banco

- Instalar o prisma em dependencia de desenvolvimento:
